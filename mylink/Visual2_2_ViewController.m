...
@interface Visual2_2_ViewController ()
-(void)drawResultImgs;
...
@implementation Visual2_2_ViewController
@synthesize first, linkResult_array2, linkNames_array2, dup_id, pageIndex;
- (void)viewDidLoad {
    [super viewDidLoad];
    ...
    //pageViewController.view.frame = CGRectMake(0, 0, self.view.frame.size.width, self.view.frame.size.height-80); //Change the size of page view controller for paging indicator
...
-(void)drawResultImgs
{
    ...
    {
        numOfImgs += (int)[[array objectAtIndex:0] count];
    }
    ...
    id_array = [NSMutableArray arrayWithCapacity:numOfImgs];
    out_array = [NSMutableArray arrayWithCapacity:numOfImgs];
    //[1]Draw image
    ...
    {
        cnt++;
        int numOfImgs2 = (int)[[array objectAtIndex:0] count];
        for(int i=0; i<numOfImgs2; i++)
        {
            NSMutableArray *pos_array2 = [NSMutableArray arrayWithCapacity:([[[array objectAtIndex:0] objectAtIndex:i] count]+1)];
            float nodeNum = (float)([[[array objectAtIndex:0] objectAtIndex:i] count]-1); //s와 e를 제외한 개수
            ...
            NSArray *resultId_array = [[array objectAtIndex:0] objectAtIndex:i];
            NSArray *resultOut_array = [[array objectAtIndex:1] objectAtIndex:i];
            NSArray *resultIn_array = [[array objectAtIndex:2] objectAtIndex:i];
            //(2)arrow & node
            for(int j=0; j<(int)nodeNum; j++)
            {
                //out & in
                NSString *str_out = [resultOut_array objectAtIndex:j];
                ...
                NSString *str_in = [resultIn_array objectAtIndex:j];
                ...
                [path stroke];
                //중복 노드인지 체크
                if([[resultId_array objectAtIndex:j] isEqualToString:dup_id])
                {
                    [[UIColor redColor] setStroke];
                    path = [UIBezierPath bezierPathWithRect:CGRectMake(curX-12.0f, curY-7.0f, 84.0f, 84.0f)];
                    [path setLineWidth:2.0f];
                    [path stroke];
                    [[UIColor grayColor] setStroke];
                }
            ...
            [path fill];
            [path stroke];
            [[linkNames_array2 objectAtIndex:cnt] drawInRect:CGRectMake(curX-40, curY+80.0f, 140.0f, 20.0f) withAttributes:att_title];
            UIImage *resultImg = UIGraphicsGetImageFromCurrentImageContext();
            UIGraphicsEndImageContext();
            [img_array addObject:resultImg];
            [pos_array addObject:pos_array2];
            [id_array addObject:[[array objectAtIndex:0] objectAtIndex:i]];
            [out_array addObject:[[array objectAtIndex:1] objectAtIndex:i]];
        }
    }
    ...
    for(NSArray *array in linkResult_array2)
    {
        NSArray *ids_array = [array objectAtIndex:0];
    ...
    {
        NSArray *ids_array = [array objectAtIndex:0];
        for(NSArray *ids_array2 in ids_array)
        {
            cnt++;
            UIImage *resultImg = [img_array objectAtIndex:cnt];
            float width = resultImg.size.width;
            float height = resultImg.size.height;
            if([UIScreen instancesRespondToSelector:@selector(scale)] && [[UIScreen mainScreen] scale] == 2.0f) UIGraphicsBeginImageContextWithOptions(CGSizeMake(width,height), NO, 2.0f);
            else if([UIScreen instancesRespondToSelector:@selector(scale)] && [[UIScreen mainScreen] scale] == 3.0f) UIGraphicsBeginImageContextWithOptions(CGSizeMake(width,height), NO, 3.0f);
            ...
            //me
            //(1)position
            float x = [[[[pos_array objectAtIndex:cnt] objectAtIndex:0] objectAtIndex:0] floatValue];
            float y = [[[[pos_array objectAtIndex:cnt] objectAtIndex:0] objectAtIndex:1] floatValue];
            ...
            for(int i=0; i<[ids_array2 count]; i++)
            {
                ...
                //(1)position
                float x = [[[[pos_array objectAtIndex:cnt] objectAtIndex:i+1] objectAtIndex:0] floatValue];
                float y = [[[[pos_array objectAtIndex:cnt] objectAtIndex:i+1] objectAtIndex:1] floatValue];
            ...
            UIGraphicsEndImageContext();
            [img_array replaceObjectAtIndex:cnt withObject:resultImg];
        }
    }
...
-(PageContentViewController *)viewControllerAtIndex:(NSUInteger)index
{
    ...
    pageContentViewController.visual1 = nil;
    pageContentViewController.visual2_2 = self;
    ...
    pageContentViewController.ids_array = [NSMutableArray arrayWithArray:[id_array objectAtIndex:index]];
    ...
    pageContentViewController.out_array = [NSArray arrayWithArray:[out_array objectAtIndex:index]];
